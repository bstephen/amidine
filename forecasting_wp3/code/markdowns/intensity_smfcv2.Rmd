...
- CONVERGENCE! test models for convergence...

As shown in the plots previously, modeling the aggregate level peak demand will be quite different to forecasting at the SM level
...
The pits are obv a bit all over the place, we don't have enough data to properly evaluate this way...
...
- is this hypothetical hierarchy truly representative? must have massive impact on results
- we saw similar results previously - there is obv high correlation on the hierarchy structure
- there are correlation 'blocks'
- there is fairly low correlation at the feeder level
- is this representative in practice? this is saying that feeder demand is uncorrelated basically, because of random SM sampling assignment, but is this truly representative?
- e.g. wealthier areas with higher demand will typically serve the same ss
- e.g. neighbor effect of buying EV/battery etc.
- oxford study, WP clustering of similar profiles

- from the results anyway, I think we could safely split up the copula?
...
<!-- ```{r fig.height=3,fig.width=15,message=FALSE,warning=FALSE,results='hide'} -->


<!-- sample_ids <- pklcl_data[aggregation!="sm",sort(unique(id))] -->
<!-- par(mar=c(3,3,1.5,1))  # Trim margin around plot [b,l,t,r] -->
<!-- par(tcl=0.35)  # Switch tick marks to insides of axes -->
<!-- par(mgp=c(1.5,0.2,0))  # Set margin lines; default c(3,1,0) [title,labels,line] -->
<!-- par(xaxs="r",yaxs="r")  # Extend axis limits by 4% ("i" does no extension) -->
...
<!-- sum(sapply(agg_m3,function(z){sum(sapply(z,function(x){x$converged}))==3}))==length(agg_m3) -->

<!-- sapply(agg_m3$ps1,function(x){x$converged}) -->

<!-- plot(agg_m3$ps1$Test) -->
...
- nb: checked monotonic penalised splines for fixing negative linear relationship - no help
- nb: checked random() for factor shrinkage, need to understand better :s - no help
- nb: checked ri() - couldn't get to work
- nb: pbv(lagged_demand, dow_ftr) - no help

next steps and development...
- benchmarks ETS bootstraps/ARIMA gaussian/ARMA-GARCH
- GB2 family?
- interactions! variance and lagged demand?
- more lags (2,3,15) from ACF --->
- temperature variable?
- we're missing the peaks when it's freezing cold
- temp forecasts? freezing/not freezing cat.
- midas data...ecmwf forecasts averaged over london? or min-of-mins?
- boosting would help with variable selection on per-node basis - pbz() --- gamboostLSS! --- xgboostLSS? --- https://github.com/StatMixedML/XGBoostLSS
- holidays?
...
<!-- - we will use the truncated normal as our conditional distribution for a gamlss -->
- since we are close to zero now, we need to consider this rather than rely on the thin tails of the gaussian as before :)
<!-- - some nodes (e.g. N0304) clearly has a sort of minimum peak demand level (0.4kWh) whereas others can have potentially v. small minimum peak demand -->
...
# ggplot(data=melt(pklcl_data[id%in%smp_ids,.(id,demand,log_demand=log(demand))],measure.vars =2:3))+
#   labs(x = "max demand [kWh]")+
#   geom_histogram(aes(value),bins=100)+
#   facet_grid(variable~id,scales = "free")
...
- these give an idea of what could be a useful conditional distribution I've found empirically
- the BCPE has a tendency to fail in my experience
- we will try the GB2 distribution, we have prior experience and it worked really well
- truncated gaussian was also tried (not shown), but was failing at some nodes
...
- the extra parameters hlp clearly at this node to reduce the uncertainty
...
- I don't think it makes sense to over-engineer the location shape and scale parameters for the GB2, because the $\mu$ value already effects the shape of the distribution
...
<!-- tmp <- pklcl_data[id%in%c("N0003","N2206")] -->

<!-- save(tmp,file="tmp.rda") -->

<!-- #  -->
<!-- # fwrite(lcl_data[aggregation=="sm",.(date_time_utc=date_time,id,demand)],file = "../data/lowcarbonlondon/ukds/lcl_clean.csv", -->
...
# next steps/notes
- gamboostLSS? could be good for feature selection, and has better base learner features
- seasonality 
- tod_peaklag
- mode(demand)
- markov switching for regimes, mixture model
- grouping of sms might help holiday definition, logistic for predicting prob of holiday on whole grup ---> features



- analog ensemble?
- EVT gam
- hierarchy with copula for each level take samples and enforce coherency on samples?

https://github.com/anastasiospanagiotelis/ProbReco
https://robjhyndman.com/publications/coherentprob/
https://www.monash.edu/business/ebs/research/publications/ebs/wp02-2019.pdf
https://reader.elsevier.com/reader/sd/pii/S0167947311000971?token=F2B42952A4548AFA3EC51F6AA409FA6ED48C7CD3C8EB6D394BBC54DE49967A674927D35D15C3EC1D5A4216324279D84A

- bottom-up/ top-down https://cran.r-project.org/web/packages/hts/vignettes/hts.pdf,https://www.lancaster.ac.uk/~morganle/images/HierarchicalModels.pdf https://cloud.r-project.org/web/packages/fable/index.html
- VAR for forecast reconciliation https://robjhyndman.com/talks/Hierarchical.pdf (23)


- covariances
- literature review peaks
- literature review LCL data


- gam multi-resolution EDF, advanced tensor effects?
-- ordinal categorical (time-of-peak) https://explorablemultiverse.github.io/examples/likert/R/analysis.html
-- generalised extreme value - peak intensity

-- functional regression? scalar (out) function(adaily lags)



https://fromthebottomoftheheap.net/2014/05/09/modelling-seasonal-data-with-gam/
https://link.springer.com/content/pdf/10.1186/s12874-019-0666-3.pdf
